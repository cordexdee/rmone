...
namespace uGovernIT.Manager
{
    ...
    public class ReportGenerationHelper
    {
        ...
        public string GroupHeadingSufix { get; set; }
        public string GrandGroupHeading { get; set; }
        ...
        void InitTotalSummaries(ASPxGridView aspxGV)
        {
            if (aspxGV.TotalSummary.Count > 0)
            {
                ...
                {
                    Borders = BorderSide.None, //BorderSide.Left | BorderSide.Right | BorderSide.Bottom,
                    //SizeF = new SizeF(report.PageWidth - (report.Margins.Left + report.Margins.Right), bandHeight + 2), /*Borders = BorderSide.Bottom,*/
                ...
                if (!string.IsNullOrWhiteSpace(GrandGroupHeading))
                {
                    XRLabel label = new XRLabel();
                    label.Borders = BorderSide.None;
                    label.TextAlignment = TextAlignment.MiddleRight;
                    label.Text = GrandGroupHeading;
                    label.Font = new Font(label.Font.FontFamily, Fontsize + 1, FontStyle.Bold);
                ...
                foreach (ASPxSummaryItem item in aspxGV.TotalSummary)
                {
                    ...
                    if (col != null)
                    {
                        if (detailsInfo.Contains(col))
                        {
                            ...
                            if (item.SummaryType.ToString().Equals("sum", StringComparison.InvariantCultureIgnoreCase))
                            {
                                label.Summary.FormatString = label.Summary.FormatString.Replace($"{item.SummaryType.ToString()} = ", string.Empty);
                            }
        ...
        void InitGroupHeaders(ASPxGridView aspxGV)
        {
            ...
            for (int i = groupedColumns.Count - 1; i >= 0; i--)
            {
                ...
                XRPanel pnl = new XRPanel()
                {
                    ...
                    SizeF = new SizeF(report.PageWidth - (report.Margins.Left + report.Margins.Right) - ((initialGroupOffset + i * 21)), bandHeight), /*Borders = BorderSide.Bottom,*/
                    Borders = BorderSide.None  //BorderSide.Left | BorderSide.Right
                };
                ///for footer Summary
                if (aspxGV.GroupSummary.Count > 0)
                {
                    if (!string.IsNullOrWhiteSpace(GroupHeadingSufix))
                    {
                        XRLabel label = new XRLabel();
                        label.Borders = BorderSide.None;
                        label.TextAlignment = TextAlignment.MiddleRight;
                        label.Text = groupedColumn.Caption + " " + GroupHeadingSufix;
                        label.Font = new Font(label.Font.FontFamily, Fontsize + 1, FontStyle.Bold);
                ...
                {
                    Padding = new PaddingInfo(0, 0, 0, 5),
                ...
                else
                {
                    ...
                    {                        
                        l.Text = (new string(Enumerable.Range(1, (i * 3)).Select(x => ' ').ToArray())) + groupedColumn.Caption + ": [" + groupedColumn.FieldName + "]";
                    }
                }
                //l.LocationF = new PointF(initialGroupOffset + i * 10, 0);
                l.LocationF = new PointF(initialGroupOffset, 0);
                ...
                //l.SizeF = new SizeF((headerTable.WidthF - (initialGroupOffset + i * 10)), bandHeight);
                //}
                //pnl.Controls.Add(l);
                gb.Controls.Add(l);
        ...
        void InitDetailsAndPageHeaderAndFooter(ASPxGridView aspxGV, ReportQueryFormat qryFormat)
        {
            ...
            for (int i = 0; i < columns.Count; i++)
            {
                if (columns[i].IsVisible)
                {
                    if (!groupedColumns.Contains(columns[i].GridViewColumn))
                    {
                        ...
                        {
                            if (columns[i].GridViewColumn.PropertiesEdit != null)
                            {
            ...
            XRPanel pnlheader = new XRPanel() { HeightF = bandHeight + 1, LocationF = new PointF(0, 59), Borders = BorderSide.None, WidthF = pagewidth };
            //pnlheader.Controls.AddRange(lbList.ToArray());
            pnlheader.Controls.Add(headerTable);
            XRPanel pnldetails = new XRPanel() { HeightF = bandHeight, Borders = BorderSide.None, WidthF = pagewidth };
            ...
            logo.ImageUrl = uHelper.GetReportLogoURL(HttpContext.Current.GetManagerContext());
        ...
        private void picBox_BeforePrint(object sender, PrintEventArgs e)
        {
            XRPictureBox picBox = sender as XRPictureBox;
            if (picBox != null)
            {
                XRTableCell parentCell = picBox.Parent as XRTableCell;
                if (parentCell != null)
                {
                    parentCell.TextAlignment = TextAlignment.MiddleCenter;
                    picBox.HeightF = parentCell.HeightF + 4;
                    picBox.WidthF = parentCell.WidthF;
                    picBox.Sizing = DevExpress.XtraPrinting.ImageSizeMode.Squeeze;
                    picBox.ImageAlignment = ImageAlignment.MiddleCenter;
                }
            }
        }
        ...
        void cell2_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            if (this.queryFormat != null && this.queryFormat.Legend != null)
            {
                ...
                if (!string.IsNullOrEmpty(cell.Text) && cell.Text.Contains(Constants.Separator))
                {
                    ...
                    {
                        string[] vals = cell.Text.Split(new string[] { Constants.Separator1 }, StringSplitOptions.RemoveEmptyEntries);
                        string[] items = null;
                        string colorCode;
                        cell.Text = "";
                        ...
                        {
                            items = val.Split(new string[] { Constants.Separator }, StringSplitOptions.RemoveEmptyEntries);
                            if (items.Length > 1)
                            {
                                colorCode = "#FFFFFF";
                                colorCode = this.queryFormat.Legend[items[0]];
                                if (FileFormat == "pdf")
                                    cell.Text = cell.Text + "<backcolor=" + colorCode + ">" + items[1] + "</backcolor><br>";
                                else
                                    cell.Text = cell.Text + "<b><color=" + colorCode + ">" + items[1] + "</color></b><br>";
        ...
        private List<ColumnInfo> GetColumnsInfo(ASPxGridView aspxGV, int pagewidth)
        {
            ...
            foreach (GridViewDataColumn dataColumn in visibleColumns)
            {
                ...
                ColumnInfo column = new ColumnInfo(dataColumn) { ColumnCaption = string.IsNullOrEmpty(dataColumn.Caption) ? dataColumn.FieldName : dataColumn.Caption, ColumnWidth = width, FieldName = dataColumn.FieldName, IsVisible = true };
            ...
            foreach (GridViewColumn column in aspxGV.VisibleColumns)
            {
                if (column is GridViewDataColumn)
                {
                    if (column.ParentBand != null)
                    {
                        ...
                        if (string.IsNullOrEmpty(column.ParentBand.Name))
                            ((GridViewDataColumn)column).FieldName = column.Name;
                        else
                            ((GridViewDataColumn)column).FieldName = column.Name + "-" + column.ParentBand.Name;
                    }
    ...
    public class DynamicReportBuilderHelper
    {
        ...
        ProjectReportEntity prEntity;
        NPRProjectReportEntity nprprEntity;
        //Dictionary<string, string> dictMonitorState = null;
        string monitorOnBudgetTitle = string.Empty;
        string monitorIssueTitle = string.Empty;
        string monitorRiskTitle = string.Empty;
        string monitorScopeTitle = string.Empty;
        string monitorOnTimeTitle = string.Empty;

        public void GenerateReport(XtraReport r, ProjectReportEntity prEntity)
        {
        ...
        private void InitDetailsBasedOnXRTableCell(XtraReport rep, List<DataSourceDefinition> dsd)
        {
            ...
            int fWidth = 0;
            int monitorWidth = 0;
            Font arialBold = new Font("Verdana", 8.25F);
            List<Tuple<string, int>> lstMonitorColumnsIndividulaMaxWidth = new List<Tuple<string, int>>();
            if (prEntity != null && prEntity.ShowMonitorState)
            {
                List<string> lsttest = new List<string>() { monitorIssueTitle, monitorScopeTitle, monitorOnBudgetTitle, monitorOnTimeTitle, monitorRiskTitle };
                String text1 = string.Empty;
                int monitorPadding = 2;
                foreach (string str in lsttest)
                {
                    Size textSize = TextRenderer.MeasureText(str, arialBold);
                    if (textSize != null)
                    {
                        monitorWidth += textSize.Width + monitorPadding;
                        lstMonitorColumnsIndividulaMaxWidth.Add(new Tuple<string, int>(str, textSize.Width + monitorPadding));
                    }
                }

                for (int i = 0; i < dsd.Count - 1; i++)
                    totalf += dsd[i].Factor;

                fWidth = (rep.PageWidth - monitorWidth - ReportOffset - (rep.Margins.Left + rep.Margins.Right)) / totalf;
            }
            else
            {
                ...
                fWidth = (rep.PageWidth - ReportOffset - (rep.Margins.Left + rep.Margins.Right)) / totalf;
            }
            ...
            for (int i = 0; i < colCount; i++)
            {
                ...
                {
                    if (prEntity.ModuleMonitorsData != null)
                    {
                        foreach (DataRow mRow in prEntity.ModuleMonitorsData)
                        {
                            XRTableCell xrHeaderTableCellIss = new XRTableCell();
                            xrHeaderTableCellIss.Name = Convert.ToString(mRow[DatabaseObjects.Columns.ProjectMonitorName]);
                            xrHeaderTableCellIss.Text = Convert.ToString(mRow[DatabaseObjects.Columns.Title]);
                            xrHeaderTableCellIss.TextAlignment = dsd[i].TextAlignment;
                            Size tSize = TextRenderer.MeasureText(Convert.ToString(mRow[DatabaseObjects.Columns.Title]), arialBold);
                            float width = tSize.Width;
                            xrHeaderTableCellIss.SizeF = new SizeF(width, 20);
                            xrHeaderTableCellIss.Tag = "Monitors";
                            xrHeaderTableCellIss.StylePriority.UsePadding = false;
                            xrHeaderTableCellIss.Padding = new PaddingInfo(0, 0, 0, 0, 100);
                            headers.Add(xrHeaderTableCellIss);
                        }
                    }
                }
                else
                {
                    DataSourceDefinition dsdpass = dsd[i];
                    Font verdana = new Font("Verdana", 8.25F);
                    Size textSize = TextRenderer.MeasureText(dsdpass.CaptionName, verdana);
                    int updateWidth = fWidth;
                    if (dsdpass.Fieldname == "ShowProjectName" || dsdpass.Fieldname == DatabaseObjects.Columns.Title)
                        updateWidth = 150; // (updateWidth * dsdpass.Factor) - 46;
                    //else if (dsdpass.Fieldname == "ShowDescription")
                    //    updateWidth = 200;
                    else if (dsdpass.Fieldname == "ShowPercentComplete")
                        updateWidth = 40;
                    else if (dsdpass.Fieldname == "ShowProjectManagers" || dsdpass.Fieldname == "ShowTargetDate")
                        updateWidth = 85;
                    else if (textSize != null && dsdpass.Fieldname != "ShowDescription" && dsdpass.Fieldname != "ShowProStatus" && dsdpass.Fieldname != "ShowIssues" && dsdpass.Fieldname != "ShowRisk")
                        updateWidth = textSize.Width + 5;
                    else
                        updateWidth = (updateWidth * dsdpass.Factor) - 20;

                    XRTableCell xrHeaderTableCell = CreateHeaderTableCell(dsdpass, updateWidth, incShift);
            ...
            for (int i = 0; i < colCount; i++)
            {
                if (dsd[i].Fieldname == "ShowMonitorState")
                {
                    if (prEntity.ModuleMonitorsData != null)
                    {
                        foreach (DataRow mRow in prEntity.ModuleMonitorsData)
                        {
                            ...
                            xrTableCellIss.Name = Convert.ToString(mRow[DatabaseObjects.Columns.ProjectMonitorName]);
                            ...
                            xrTableCellIss.StylePriority.UsePadding = false;
                            xrTableCellIss.BeforePrint += xrpic_BeforePrint;
                            XRPictureBox xrPB1 = new XRPictureBox();
                            xrPB1.Name = "xrPB1";
                            xrPB1.SizeF = new SizeF(width, 20);
                            xrPB1.ImageAlignment = ImageAlignment.MiddleCenter;
                            xrPB1.Borders = DevExpress.XtraPrinting.BorderSide.None;
                            xrPB1.StylePriority.UseBorders = false;
                            xrPB1.Sizing = DevExpress.XtraPrinting.ImageSizeMode.Squeeze;
                            xrPB1.Padding = new PaddingInfo(0, 0, 5, 0, 100);
                            ...
                            xrLabeliis.SizeF = new SizeF(width, 20);
                            xrTableCellIss.Controls.Add(xrPB1);
                            xrTableCellIss.Controls.Add(xrLabeliis);
                            xrTableCellIss.Tag = "Monitors";

                            details.Add(xrTableCellIss);
                        }
                    }
                }
                else
                {
                    ...
                    XRTableCell xrDetailTableCell = CreateDetailTableCell(dsdpass, updateWidth, incShift);
                    incShift += fWidth * dsd[i].Factor;
        ...
        private void xrpic_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            XRTableCell picCell = sender as XRTableCell;
            if (picCell != null)
            {
                if (picCell.HasChildren)
                {
                    XRPictureBox picBox = picCell.FindControl("xrpb1", true) as XRPictureBox;
                    if (picBox != null)
                    {
                        //reducing width by 1point to keep right border visible
                        picBox.WidthF = picCell.WidthF - 1;
                    }
                }
            }
        }
        ...
        private XRTableCell CreateHeaderTableCell(DataSourceDefinition dsd, int fWidth, int incShift)
        {
            ...
            if (dsd.Fieldname == "ShowPriority")
                xrTableCell.SizeF = new SizeF(fWidth + 10, 20);
            else if (dsd.Fieldname == "ShowProgress")
                xrTableCell.SizeF = new SizeF(fWidth + 18, 20);
            else if (dsd.Fieldname == "ShowTargetDate" || dsd.Fieldname == "ShowDepartment" || dsd.Fieldname == "ShowProjectManagers"
                || dsd.Fieldname == "ShowAccomplishment")
                xrTableCell.SizeF = new SizeF(fWidth + 5, 20);
            else
                xrTableCell.SizeF = new SizeF(fWidth, 20);
            return xrTableCell;
        }
        ...
        private XRTableCell CreateDetailTableCell(DataSourceDefinition dsd, int fWidth, int incShift)
        {
            ...
            {
                xrTableCell.SizeF = new SizeF(fWidth + 10, 20);
                xrTableCell.Padding = new PaddingInfo(2, 2, 10, 0);
                xrTableCell.TextAlignment = TextAlignment.TopCenter;
            ...
            {
                xrTableCell.Padding = new PaddingInfo(2, 2, 10, 0);
                xrTableCell.TextAlignment = TextAlignment.TopLeft;
            ...
            else if (dsd.Fieldname == "ShowDepartment")
            {
                xrTableCell.SizeF = new SizeF(fWidth + 5, 20);
                xrTableCell.Padding = new PaddingInfo(2, 2, 10, 0);
                xrTableCell.TextAlignment = TextAlignment.TopLeft;
                xrTableCell.DataBindings.Add("Text", null, DatabaseObjects.Columns.TicketBeneficiaries);
            }
            else if (dsd.Fieldname == "ShowFunctionalArea")
            {
                xrTableCell.Padding = new PaddingInfo(2, 2, 10, 0);
                xrTableCell.TextAlignment = TextAlignment.TopLeft;
                xrTableCell.DataBindings.Add("Text", null, DatabaseObjects.Columns.FunctionalAreaLookup);
            }
            else if (dsd.Fieldname == "ShowProgress")
            {
                xrTableCell.SizeF = new SizeF(fWidth + 18, 20);
            ...
            else if (dsd.Fieldname == "ShowAccomplishment")
            {
                ...
                richText.LocationFloat = new DevExpress.Utils.PointFloat(0F, 0F);
                //richText.SizeF = new SizeF(fWidth * dsd.Factor, 20);
                richText.SizeF = new SizeF(fWidth, 20);
                xrTableCell.Name = dsd.Fieldname;
                xrTableCell.Controls.AddRange(new DevExpress.XtraReports.UI.XRControl[] { richText });
            }
            ...
            else if (dsd.Fieldname == "TicketDesiredCompletionDate" || dsd.Fieldname == "TicketCreationDate")
            {
                xrTableCell.Padding = new PaddingInfo(2, 2, 10, 0);
                xrTableCell.TextAlignment = TextAlignment.TopCenter;
                xrTableCell.DataBindings.Add("Text", null, dsd.Fieldname, "{0:MMM-dd-yyyy}");
                xrTableCell.Name = dsd.Fieldname;
            }
            else
            {
