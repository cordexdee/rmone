...
namespace uGovernIT.DxReport
{
    ...
    public class OnePagerReportHelper
    {
        ...
        public OnePagerReportHelper(ApplicationContext applicationContext)
        {
            ...
            pmmList = null; // ticketManager.GetAllTickets(module); ; // GetTableDataManager.GetTableData(DatabaseObjects.Tables.PMMProjects, $"{DatabaseObjects.Columns.TenantID}='{context.TenantID}'");
        }
        ...
        public ProjectCompactReportEntity GetOnePagerReportEntity(string ticketStatus = "Open")
        {
            UGITModule module = ModuleManager.LoadByName(ModuleNames.PMM, true);
            if (ticketStatus == "Open")
            { 
                pmmList = ticketManager.GetOpenTickets(module); }
            else if(ticketStatus == "Closed")
            { 
                pmmList = ticketManager.GetClosedTickets(module); }
            else
                pmmList = ticketManager.GetAllTickets(module);
        ...
        protected void GetProjectDetails()
        {
            ...
            if (PMMIds != null && PMMIds.Length > 0)
            {
                ...
                //queryexpression = $"{DatabaseObjects.Columns.TicketId} IN (" + string.Join(",", PMMIds) + ")";
                //}
            }
            ...
            if (spListItemColl != null && spListItemColl.Count() > 0)
            {
                ...
                foreach (DataRow dr in sourcetable.Rows)
                {
                    ...
                    commonexpressions.Add(string.Format("'{1}'",
                                                       DatabaseObjects.Columns.TicketId, dr[DatabaseObjects.Columns.TicketId]));
                ...
                queryexpression = $"{DatabaseObjects.Columns.TicketId} IN (" + string.Join(",", commonexpressions) + ")";

                // DataTable convertedSourcetable = uHelper.ConvertTableLookupValues(context, sourcetable);
                DataTable dtProject = sourcetable.Copy();
            ...
            if (entity.ProjectDetails.Rows.Count > 0)
            {
                ...
                foreach (DataRow row in entity.ProjectDetails.Rows)
                {
                    ...
                    //sumTasks = uGITTaskManager.GetSummaryTasksByProjectID("PMM", Convert.ToString(row[DatabaseObjects.Columns.TicketId]));
                    ////string query = string.Format("{0} = '{1}'", DatabaseObjects.Columns.TaskBehaviour, "Milestone");
                    //if (sumTasks != null && sumTasks.Rows.Count > 0)
                    //{
                    //    //dv = new DataView(sumTasks);

                    //    //dv.RowFilter = query; // query
                    //    //sumTasks = dv.ToTable();
                    //    if (summaryTasks != null && summaryTasks.Rows.Count > 0)
                    //        summaryTasks.Merge(sumTasks);
                    //    else
                    //        summaryTasks = sumTasks.Copy();
            ...
            if (summaryTasks != null && summaryTasks.Rows.Count > 0)
            {
                ...
                else
                {
                    ...
                    {
                        if (UGITUtility.StringToInt(sRow[DatabaseObjects.Columns.ParentTaskID]) > 0)
                        {
                            int parentID = UGITUtility.StringToInt(sRow[DatabaseObjects.Columns.ParentTaskID]);
                            bool keepLooping = true;
                            do
                            {
                                keepLooping = false;
                                DataRow spRow = entity.Tasks.AsEnumerable().FirstOrDefault(x => x.Field<string>(DatabaseObjects.Columns.Id) == parentID.ToString());
                                if (spRow != null)
                                {
                                    parentID = UGITUtility.StringToInt(spRow[DatabaseObjects.Columns.ParentTaskID]);
                                    if (parentID > 0)
                                    {
