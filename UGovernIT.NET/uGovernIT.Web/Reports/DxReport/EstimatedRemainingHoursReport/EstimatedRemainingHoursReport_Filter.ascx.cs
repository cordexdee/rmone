...
namespace uGovernIT.DxReport
{
    public partial class EstimatedRemainingHoursReport_Filter : UserControl
    {
        ...
        public DataTable GetProjectTasks()
        {
            ...
            if (taskTable.Columns.Contains("AssignedToUser$"))
            {
                taskTable.Columns["AssignedToUser"].ColumnName = "AssignedToUser_";
                taskTable.Columns["AssignedToUser$"].ColumnName = "AssignedToUser";
            }
            //added new column.
        ...
        protected string GetTitleHtmlData()
        {
           ...
           // data.Append("<div style=\"float:left;");
            ...
            if (childCount > 0)
            {
                data.AppendFormat("<img style='float:left;padding-right:2px;' src='/content/images/minimise.gif' colexpand='true' onclick=\"event.cancelBubble=true; CloseChildren('{0}' , '{1}', this)\" />", Eval(DatabaseObjects.Columns.ItemOrder), Eval("ID"));
            }
            if (Eval("Title") != null)
            {
