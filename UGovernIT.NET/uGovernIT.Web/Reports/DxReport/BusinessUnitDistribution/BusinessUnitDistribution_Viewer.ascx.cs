...
{
    public partial class BusinessUnitDistribution_Viewer : UserControl
    {
        ...
        protected override void OnInit(EventArgs e)
        {
            ...
            {
                ReportFilterURl = delegateControl + "?reportName=BusinessUnitDistribution&Filter=Filter&Module=" + Module + "&title=" + ReportTitle;
        ...
        private void GenerateColumns()
        {
            ...
            //colId.CellStyle.HorizontalAlign = HorizontalAlign.Left;
            //colId.HeaderStyle.HorizontalAlign = HorizontalAlign.Center;
            //colId.FieldName = DatabaseObjects.Columns.BCCISector;
            //colId.Caption = "BCCI Sector";
            //colId.Settings.AllowHeaderFilter = DevExpress.Utils.DefaultBoolean.Default;
            //colId.SettingsHeaderFilter.Mode = GridHeaderFilterMode.CheckedList;
            //colId.GroupIndex = 2;
            //grdReport.Columns.Add(colId);
        ...
        private DataTable LoadData()
        {
            ...
            if (!dtResult.Columns.Contains(DatabaseObjects.Columns.BCCISector))
                dtResult.Columns.Add(DatabaseObjects.Columns.BCCISector);
            ...
            if (dtOPM != null && dtOPM.Rows.Count > 0)
            {
                foreach (DataRow dr in dtOPM.Rows)
                {
                    ...
                    if (lstStudios != null && lstStudios.Count > 0)
                        newDR[DatabaseObjects.Columns.StudioLookup] = lstStudios.FirstOrDefault(x => x.ID == UGITUtility.StringToLong(dr[DatabaseObjects.Columns.StudioLookup]))?.Title;
                    ...
                    newDR[DatabaseObjects.Columns.BCCISector] = Convert.ToString(dr[DatabaseObjects.Columns.BCCISector]);
            ...
            if (dtCPR != null)
            {
                ...
                if (drsCPR != null && drsCPR.Length > 0)
                {
                    foreach (DataRow dr in drsCPR)
                    {
                        ...
                        if(drCollection == null || drCollection.Length == 0) { continue; }
            ...
            if (!string.IsNullOrEmpty(Divisions))
            {
                ...
                Divisions = Divisions.Replace("None", string.Empty);
                drResults = dtResult.Select($"{DatabaseObjects.Columns.DivisionLookup} in ({string.Join(",", Divisions.Split(',').Select(x => string.Format("'{0}'", x)).ToList())})");
        ...
        protected void btnExcelExport_Click(object sender, EventArgs e)
        {
            ...
            XtraReport report = reportHelper.GenerateReport(grdReport, (DataTable)grdReport.DataSource, "Division Distribution Report", 8F, "xls", null, qFormat);
            reportHelper.WriteXlsToResponse(Response, "DivisionDistributionReport" + ".xls", System.Net.Mime.DispositionTypeNames.Attachment.ToString());
        }
        ...
        protected void btnPdfExport_Click(object sender, EventArgs e)
        {
            ...
            XtraReport report = reportHelper.GenerateReport(grdReport, (DataTable)grdReport.DataSource, "Division Distribution Report", 6.75F, "pdf", null, qFormat);
            reportHelper.WritePdfToResponse(Response, "DivisionDistributionReport" + ".pdf", System.Net.Mime.DispositionTypeNames.Attachment.ToString());
        }
