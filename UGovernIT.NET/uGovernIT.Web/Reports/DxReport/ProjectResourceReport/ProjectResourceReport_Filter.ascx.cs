...
namespace uGovernIT.DxReport
{
    public partial class ProjectResourceReport_Filter : UserControl
    {
        ...
        private DataTable GetProjectResourceWorksByWeek()
        {
            ...
            {
                if (weekSummary.Columns.Contains(DatabaseObjects.Columns.Resource))
                {
                    weekSummary.Columns[DatabaseObjects.Columns.Resource].ColumnName = DatabaseObjects.Columns.Resource + "_";
                    weekSummary.Columns[DatabaseObjects.Columns.Resource + "$"].ColumnName = DatabaseObjects.Columns.Resource;
                }
                ...
                if (startYear > 0 && endYear > 0 && startWeek > 0 && endWeek > 0)
                {
                    ...
                    DataRow[] selectedRows = weekSummary.AsEnumerable().Where(x => x.ItemArray[workItemColumnIndex] != null && x.ItemArray[weekStartDateColumnIndex] != null).Where(x => x.Field<string>(DatabaseObjects.Columns.WorkItem) == projectPublicID
                        ...
                        && x.Field<DateTime>(DatabaseObjects.Columns.WeekStartDate) <= endMonday && x.Field<string>(DatabaseObjects.Columns.Resource + "_") != Guid.Empty.ToString()).ToArray();
        ...
        private DataTable GetProjectResourceWorksByMonth()
        {
            ...
            if (monthSummary.Columns.Contains(DatabaseObjects.Columns.Resource))
            {
                monthSummary.Columns[DatabaseObjects.Columns.Resource].ColumnName = DatabaseObjects.Columns.Resource + "_";
                monthSummary.Columns[DatabaseObjects.Columns.Resource+ "$"].ColumnName = DatabaseObjects.Columns.Resource;
            }
            ...
            if (startYear > 0 && endYear > 0 && startMonth > 0 && endMonth > 0)
            {
                ...
                DataRow[] selectedRows = monthSummary.AsEnumerable().Where(x => x.ItemArray[workItemColumnIndex] != null && x.ItemArray[monthStartDateColumnIndex] != null).Where(x => x.Field<string>(DatabaseObjects.Columns.WorkItem) == projectPublicID && x.Field<DateTime>(DatabaseObjects.Columns.MonthStartDate) >= dtStartMonth
                    && x.Field<DateTime>(DatabaseObjects.Columns.MonthStartDate) <= dtEndMonthFirst && x.Field<string>(DatabaseObjects.Columns.Resource+"_") != Guid.Empty.ToString()).ToArray();
