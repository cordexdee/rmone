...
<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="SummaryByTechnician_Viewer.ascx.cs" Inherits="uGovernIT.ControlTemplates.uGovernIT.SummaryByTechnician_Viewer" %>
...
<script type="text/javascript">
    ...
    function loadUrl()
    {
        ASPx.xr_NavigateUrl = function (url, target) {
            
            var url = url;
            window.UgitOpenPopupDialog(url, "", "Ticket Details", '90', '90', 0);
        };
    }
...
<asp:Panel runat="server" ID="pnlReport" Width="100%">
    ...
    <asp:HiddenField runat="server" ID="hdnPDFGenerated" />
      <dx:ReportToolbar ID="RptToolBar" runat='server' Width="100%" ShowDefaultButtons='False'
                            ReportViewerID="RptOpenTicketsReport" Theme="DevEx" >
                            <Items>
                        ...
                        <dx:ReportViewer ID="RptOpenTicketsReport" runat="server" Width="100%" Height="850px" AutoSize="false">
                            <BookmarkSelectionBorder BorderStyle="None" />
                            <ClientSideEvents PageLoad="function(s,e){loadUrl();}" />
                        </dx:ReportViewer>
</asp:Panel>
