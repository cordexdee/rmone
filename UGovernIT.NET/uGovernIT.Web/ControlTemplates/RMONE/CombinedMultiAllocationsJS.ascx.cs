...
{
    public partial class CombinedMultiAllocationsJS : System.Web.UI.UserControl
    {
            ...
            if (ModuleName == "CPR" || ModuleName == "CNS" || ModuleName == "OPM")
            {
                ...
                if (UGITUtility.IfColumnExists(spListItem, DatabaseObjects.Columns.CloseoutDate))
                {
                    if (!string.IsNullOrEmpty(Convert.ToString(UGITUtility.GetSPItemValue(spListItem, DatabaseObjects.Columns.CloseoutStartDate))))
                    {
                        ...
                        hdnCloseOutEndDate.Value = CloseOutEndDateString = UGITUtility.StringToDateTime(spListItem[DatabaseObjects.Columns.CloseoutStartDate]).AddWorkingDays(uHelper.getCloseoutperiod(_context)).ToString("MMM d, yyyy");
                    }
                }
