...
namespace uGovernIT.Web
{
    public sealed class SampleAuthProvider : IAuthProvider
    {
        ...
        public async Task<string> GetUserAccessTokenAsync()
        {
            ...
            //    appId,
            //    redirectUri + segments[1] +"/" + segments[2],
            ...
            try
            {
                //AuthenticationResult result = await cca.AcquireTokenSilentAsync(scopes, await cca.GetAccountAsync(appId));
                AuthenticationResult result = await cca.AcquireTokenSilent(scopes, await cca.GetAccountAsync(appId)).ExecuteAsync();
            ...
            catch (Exception ex)
            {
                ...
                if (Convert.ToString(HttpContext.Current.Session["OpenOffice_Token"]) != null && Convert.ToString(HttpContext.Current.Session["OpenOffice_Token"]) != string.Empty)
                {
                    return Convert.ToString(HttpContext.Current.Session["OpenOffice_Token"]);
                }
