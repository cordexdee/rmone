...
(function ($) {
	...
	$.jstree.plugin("search", {
		...
		_fn : {
            ...
            //Custom function by Manish start
            searchby_attr : function (attr_name, str, skip_async) {
				...
				var s = this.get_settings().search, 
					...
					success_func = function () { };
				this.data.search.str = str;
				this.data.search.attr_name = attr_name;
				...
				if(!skip_async && s.ajax !== false && this.get_container().find(".jstree-closed:eq(0)").length > 0) {
					...
					if($.isFunction(s.ajax.url)) { s.ajax.url = s.ajax.url.call(this, str); }
					if($.isFunction(s.ajax.data)) { s.ajax.data = s.ajax.data.call(this, str); }
					if(!s.ajax.data) { s.ajax.data = { "search_id" : str }; }
					if(!s.ajax.dataType || /^json/.exec(s.ajax.dataType)) { s.ajax.dataType = "json"; }
					$.ajax(s.ajax);
					return;
				}
				...
				if(this.data.search.result.length) { this.clear_search(); }
				//this.data.search.result = this.get_container().find("a" + (this.data.languages ? "." + this.get_lang() : "" ) + ":" + (s.case_insensitive ? "jstree_contains" : "contains") + "(" + this.data.search.str + ")");
                var searchString ="#"+this.get_container()[0].id + " li["+this.data.search.attr_name+"='"+this.data.search.str+"'] a";
                this.data.search.result = $(searchString);

                    this.data.search.result = $(searchString);
                   
                    if(this.data.search.result.length>0)
                    {
                	     this.data.search.result.first().addClass("jstree-search").parents(".jstree-closed").each(function () {
					      t.open_node(this, false, true);
				        });
                        this.__callback({ nodes : this.data.search.result, str : str });
                    }
			}
              //Custom function by Manish end
            ,
