...
begin             
 ...
 Begin          
  Select b.Title as DepartmentName,a.JobTitleLookup,a.DeptLookup,           
   ...
   and a.JobtitleLookup=@JobTitleId 
   --and a.EmpCostRate=@EmpCostRate        
 ...
 Begin        
   ...
   -- where JobTitleLookup in (select ID from  JobTitle where TenantID=@TenantID and Deleted=0)        
   -- )        
     Select a.Id,a.Title as JTitle,a.ShortName as JShortName, b.DeptLookup as DepartmentLookup,c.RoleLookup, d.Title as Department,
dd.Title as Divisionname,dd.Id as DivisionIdLookup , dd.ShortName as DivShortName,b.EmpCostRate,c.BillingRate   from JobTitle a
left join JobTitleCostRateByDept b on a.ID=b.JobTitleLookup
left join RoleBillingRateByDept c on c.DepartmentLookup=b.DeptLookup
left join Department d on d.ID=b.DeptLookup
left join CompanyDivisions dd on dd.ID=d.DivisionIdLookup
where   a.TenantID=@TenantID    
and b.TenantID=@TenantID and c.TenantID=@TenantID
and d.TenantID=@TenantID and dd.TenantID=@TenantID    
 End;        
end
