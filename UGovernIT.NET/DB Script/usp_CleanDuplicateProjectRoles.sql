ALTER Procedure usp_CleanDuplicateProjectRoles
...
BEGIN
	...
	begin
        ...
        --print @sqlstr;
			...
			INSERT INTO @ids (ID)
			SELECT value
			FROM STRING_SPLIT(@ColumnValue, ',');
			-- Delete duplicate IDs except for the first occurrence
			;WITH CTE AS (
				SELECT ID,
			...
			DECLARE @stageactionuserids TABLE (ID VARCHAR(36));
			INSERT INTO @stageactionuserids (ID)
			SELECT value
			FROM STRING_SPLIT(@StageActionUsers, ',');
			-- Delete duplicate IDs except for the first occurrence
			;WITH CTE AS (
				SELECT ID,
					   ROW_NUMBER() OVER (PARTITION BY ID ORDER BY (SELECT NULL)) AS RowNum
				FROM @stageactionuserids
			)
			DELETE FROM CTE WHERE RowNum > 1;
			-- Join the unique IDs back into a comma-separated string
			SET @StageActionUsers = (
				SELECT STRING_AGG(ID, ',')
				FROM @stageactionuserids
			);
