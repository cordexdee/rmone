...
END
	...
	SET 
	  @SQLStatement = 'Select ROW_NUMBER() OVER(order by  u.Name asc) ItemOrder,
		...
		and u.UGITStartDate < ''' + CONVERT(varchar, @EndDate, 121) + ''' and u.UGITEndDate > ''' + CONVERT(varchar, @StartDate, 121) + ''
		
		if(len(@Function) > 0)
		begin
		 set @SQLStatement = @SQLStatement + ''' and u.GlobalRoleID in (Select RoleId FROM FunctionRoleMapping WHERE FunctionId in (select cast(Item as bigint) from DBO.SPLITSTRING(''' + @Function + ''','',''))) order by u.Name'
		end
		else
		begin
