...
CREATE FUNCTION [dbo].[fnGetCommittedUtilization] 
(
...
BEGIN
	...
	If @i = @EndMonth
		Begin
			INSERT INTO @tmpMonthDays (Month, Days) Values (FORMAT(DATEADD(mm, @i,-1), 'MMM'), dbo.fnGetWorkingDays(@StartDate, EOMONTH(@StartDate)))
		End
	Else
		Begin
			--set @tmpDate = CAST(YEAR(@StartDate) as varchar(4)) + '-' + CAST(@EndMonth as varchar(4)) + '-' + '01';
			INSERT INTO @tmpMonthDays (Month, Days) Values (FORMAT(DATEADD(mm, @StartMonth,-1), 'MMM'), dbo.fnGetWorkingDays(@StartDate, EOMONTH(@StartDate)))
			Set @i = @i + 1;
			While @i <= @EndMonth
			Begin
