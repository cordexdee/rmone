CREATE FUNCTION [dbo].[GetUsersIdByRoleId]
(
...
BEGIN
	...
	If @IncludeAllResources = 0
		Begin
            ...
            FROM  AspNetUsers u where u.GlobalRoleID =  @RoleId and len(@RoleId) > 0 and TenantID = @TenantId and Enabled = 1
	...
	Else
		Begin
			Set @query = STUFF((SELECT ',' + u.[Id]
            FROM  AspNetUsers u where u.GlobalRoleID =  @RoleId and len(@RoleId) > 0 and TenantID = @TenantId
			and ISNULL(u.DepartmentLookup,'0') in (SELECT Case when len(Item)=0 then Isnull(u.DepartmentLookup,'0') else Item end  FROM  DBO.SPLITSTRING(replace(@Department,';#',','), ','))
			order by u.[Name]
            FOR XML PATH(''), TYPE
            ).value('.', 'NVARCHAR(MAX)') 
