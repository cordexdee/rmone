...
CREATE FUNCTION [dbo].[fnGetEffectiveUtilization] 
(
...
BEGIN
	...
	If @i = @EndMonth
		Begin
			...
			INSERT INTO @tmpMonthDays (Month, Days) Values (FORMAT(DATEADD(mm, @i,-1), 'MMM'), dbo.fnGetWorkingDays(@tmpDate, @EndDate))
		End
	Else
		Begin
			While @i < @EndMonth
			...
			INSERT INTO @tmpMonthDays (Month, Days) Values (FORMAT(DATEADD(mm, @EndMonth,-1), 'MMM'), dbo.fnGetWorkingDays(@tmpDate, @EndDate))
		End
