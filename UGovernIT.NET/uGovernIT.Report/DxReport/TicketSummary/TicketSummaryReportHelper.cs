    ...
    public class TicketSummaryReportHelper
    {
        ...
        public XtraReport GetTicketSummaryReport()
        {
            ...
            if (_selectedModules.Length > 1)
            {
                ...
                // string[] columnNames = (from dc in mytable.Columns.Cast<DataColumn>()
                //         select dc.ColumnName).ToArray();
                // DataView dv = new DataView(mytable);
                // mytable = dv.ToTable(true, columnNames);
                //IEnumerable<DataRow> dr = mytable.AsEnumerable().Distinct();
                //mytable = dr.CopyToDataTable();
        ...
        private XtraReport CreateTicketReport(TicketStatus MTicketStatus, string module)
        {
            ...
            return GetReportData(statOpen.TabCounts[CustomFilterTab.OpenTickets.ToString()], statOpen.TabCounts[CustomFilterTab.UnAssigned.ToString()], statOpen.TabCounts[CustomFilterTab.CloseTickets.ToString()], MTicketStatus);
        }
        ...
        private XtraReport GetReportData(int OpenTicketsCount, int UnassignedTicketsCount, int ClosedTicketsCount, TicketStatus MTicketStatus)
        {
            ...
            if (lstDates != null && lstDates.Count == 2)
            {
                ...
                if (this._module == "CPR")
                {
                    ...
                    if (dtTickets != null)
                    {
                        string[] selectedColumns = new[] { "ID", "ProjectManagerUser$", "EstimatorUser$", "ProjectedBudget", "SuperintendentUser$", "EstimatedConstructionStart", "EstimatedConstructionEnd","ProjectId", "TicketDialogUrl", "Title" };
                        dtTickets = new DataView(dtTickets).ToTable(false, selectedColumns);
                        foreach (DataColumn column in dtTickets.Columns)
                        {
                            bool isLookOrUserCol = (Convert.ToString(column.ColumnName).EndsWith("Lookup") || Convert.ToString(column.ColumnName).EndsWith("User"));
                            if (isLookOrUserCol && !UGITUtility.IfColumnExists(string.Format("{0}$Id", column.ColumnName), dtTickets))
                            {
                                column.ColumnName = column.ColumnName + "$Id";
                            }

                            if (Convert.ToString(column.ColumnName).EndsWith("Lookup$") || Convert.ToString(column.ColumnName).EndsWith("User$"))
                            {
                        ...
                        //string requiredColumns = "ID;ProjectManagerUser$;Address;EstimatorUser$;ProjectedBudget;SuperintendentUser$;EstimatedConstructionStart;EstimatedConstructionEnd;ApproxContractValueRpt;ProjectId;TicketDialogUrl;Title;ModuleName;DepartmentLookup;LocationLookup";
                        //for (int x = dtTickets.Columns.Count - 1; x >= 0; x--)
                        //{
                        //    if (!requiredColumns.Contains(dtTickets.Columns[x].ColumnName))
                        //        dtTickets.Columns.Remove(dtTickets.Columns[x].ColumnName);
                        //}
                    }
                }
                else
                {
                    if (dtTickets != null)
                    {
                        ...
                        string[] selectedColumns = new[] { "ID", "Age", "RequestTypeCategory", "PriorityLookup$", "Status", "RequestorUser$", "DesiredCompletionDate", "TicketId", "TicketDialogUrl", "Title", "StageActionUsersUser$", "DepartmentLookup$", "LocationLookup$" };
                        dtTickets = new DataView(dtTickets).ToTable(false, selectedColumns);
                        ...
                        //string requiredColumns = "ID;Age;RequestTypeCategory;PriorityLookup;Status;RequestorUser;DesiredCompletionDate;TicketId;TicketDialogUrl;Title;StageActionUsersUser;ModuleName;DepartmentLookup;LocationLookup";
                        //for (int x = dtTickets.Columns.Count - 1; x >= 0; x--)
                        //{
                        //    if (!requiredColumns.Contains(dtTickets.Columns[x].ColumnName))
                        //        dtTickets.Columns.Remove(dtTickets.Columns[x].ColumnName);
                ...
                //DataTable dtTicketsConverted = uHelper.ConvertTableLookupValues(_context, dtTickets);
                //openTicketsReport = new TicketSummary_Report(MTicketStatus.ToString(), dtTicketsConverted, dtDateWiseData, dicTicketStatus, dicChartUrls, dtRequestsSummary);
                if (this._module == "CPR")
        ...
        private DataTable GetRequestsSummary()
        {
            ...
            if (dtAlltickets != null && dtAlltickets.Rows.Count > 0)
            {
                ...
                int requestsToday = dtAlltickets.AsEnumerable().Where(r => r.Field<DateTime>(DatabaseObjects.Columns.Created).Date == DateTime.Now.Date).Count();
                int requestsThisWeek = dtAlltickets.AsEnumerable().Where(r => r.Field<DateTime>(DatabaseObjects.Columns.Created).Date <= DateTime.Now.Date && r.Field<DateTime>(DatabaseObjects.Columns.Created).Date >= DateTime.Now.AddDays(-7).Date).Count();
                int requestsThisMonth = dtAlltickets.AsEnumerable().Where(r => r.Field<DateTime>(DatabaseObjects.Columns.Created).Date <= DateTime.Now.Date && r.Field<DateTime>(DatabaseObjects.Columns.Created).Date >= DateTime.Now.AddMonths(-1).Date).Count();
                int requestsOlderThanMonth = dtAlltickets.AsEnumerable().Where(r => r.Field<DateTime>(DatabaseObjects.Columns.Created).Date < DateTime.Now.AddMonths(-1).Date && (r.Field<Boolean?>(DatabaseObjects.Columns.TicketClosed) != true || r.Field<Boolean?>(DatabaseObjects.Columns.TicketClosed) == null)).Count();
        ...
        private void CreateChart(ref List<ChartEntity> lstChartEntity, DashboardPanel dashboardPanel, string title, int width, int height, int order, bool isNewLine, int ViewId)
        {
            ...
            string chartUrl = string.Format("/Report/Content/IMAGES/ugovernit/upload/{0}", fileName);
