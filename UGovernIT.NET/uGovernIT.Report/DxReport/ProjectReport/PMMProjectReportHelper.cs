    ...
    public class PMMProjectReportHelper
    {
        ...
        public TSKProjectReportEntity GetProjectReportEntity(ApplicationContext context, TSKProjectReportEntity prEntity, int[] PMMIds, string moduleName, string selectedProjectStatus = "open")
        {
            ...
            if (spListItemColl != null && spListItemColl.Length > 0)
            {
                ...
                {
                    if (sourcetable.Columns[i].ColumnName.EndsWith("Lookup") || sourcetable.Columns[i].ColumnName.EndsWith("User"))
                        sourcetable.Columns[i].ColumnName = sourcetable.Columns[i].ColumnName + "_";
                    else if (sourcetable.Columns[i].ColumnName.EndsWith("Lookup$") || sourcetable.Columns[i].ColumnName.EndsWith("User$"))
                        sourcetable.Columns[i].ColumnName = sourcetable.Columns[i].ColumnName.Remove(sourcetable.Columns[i].ColumnName.Length-1);
