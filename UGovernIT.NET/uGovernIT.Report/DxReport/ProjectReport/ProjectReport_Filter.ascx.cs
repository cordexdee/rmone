...
namespace uGovernIT.Report.DxReport
{
    public partial class ProjectReport_Filter : UserControl
    {
        ...
        protected void lnkBuild_Click(object sender, EventArgs e)
        {
            ...
            string url = _context.SiteUrl + delegateControl + "?reportName=ProjectReport&isudlg=1";
        ...
        protected void cblProject_Load()
        {
            ...
            //string tableName = DatabaseObjects.Tables.PMMProjects;
            //if (!string.IsNullOrEmpty(ModuleName))
            //{
            //    //ModuleViewManager moduleManager = new ModuleViewManager(_context);
            //    tableName = moduleManager.GetModuleTableName(ModuleName);
            //}

            //  spQuery.ViewFields = string.Format("<FieldRef Name='{0}'/><FieldRef Name='{1}'/><FieldRef Name='{2}'/>", DatabaseObjects.Columns.Id, DatabaseObjects.Columns.TicketId, DatabaseObjects.Columns.Title);
            //  spQuery.ViewFieldsOnly = true;
            //_DataTableFilter = _DataTable.Select(spQuery).CopyToDataTable();
            //DataRow[] dataRow = null;

            DataTable _DataTableFilter = new DataTable();
            var filterRows = _DataTable.Select(spQuery);
            
            _DataTableFilter = filterRows.Any() ? filterRows.CopyToDataTable() : null;
            //_DataTable = GetTableDataManager.GetTableData(tableName, $"TenantID='{_context.TenantID}' and " + spQuery);
            //dataRow = GetTableDataManager.GetTableData(DatabaseObjects.Tables.PMMProjects).Select(spQuery);
            ...
            //{
            //    dataRow = dataRow.Where(x => x.Field<string>(DatabaseObjects.Columns.TenantID) == _context.TenantID).ToArray();
