...
namespace uGovernIT.Report.DxReport
{
    public partial class ProjectStatusReport_Filter : UserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            DataTable _DataTable = ticketManager.GetOpenTickets(module);
        ...
        protected void cblProject_Load()
        {
            ...
            if (rbOpen.Checked)
            {
                ticketQuery.Add(string.Format("({0} = '{1}' OR {0} is null)", DatabaseObjects.Columns.TicketClosed, "False"));
            }
            else if (rbClose.Checked)
            {
                ticketQuery.Add(string.Format("{0}='{1}'", DatabaseObjects.Columns.TicketClosed, "True"));
            }
            ...
            if(ticketQuery.Count>0)
                openTicketQuery= "(" + string.Join(" And ", ticketQuery) + ")";

            DataTable _DataTable = new DataTable();
            _DataTable = GetTableDataManager.GetTableData(DatabaseObjects.Tables.PMMProjects,openTicketQuery);
        ...
        protected void lnkBuild_Click(object sender, EventArgs e)
        {
           ...
           // string url = "/_layouts/15/uGovernIT/delegatecontrol.aspx?isdlg=1&control=projectreportviewer";
            string url = _context.SiteUrl + delegateControl + "?reportName=ProjectStatusReport&";
        ...
        private void FillProjects()
        {
            ...
            if (rbOpen.Checked)
            {
                QueryList.Add(string.Format("({0}<>'{1}' OR {0} IS NULL)", DatabaseObjects.Columns.TicketClosed,"True"));
            }
            else if (rbClose.Checked)
            {
                QueryList.Add(string.Format("{0}='{1}'", DatabaseObjects.Columns.TicketClosed, "True"));
            }
            spQuery = "(" + string.Join(" And ", QueryList) + ")"; //string.Format("<Where>{0}</Where>", uHelper.GenerateWhereQueryWithAndOr(QueryList, QueryList.Count - 1, true));
            lstProjectsLHS.Items.Clear();

            var spListItemColl = GetTableDataManager.GetTableData(DatabaseObjects.Tables.PMMProjects, $"{DatabaseObjects.Columns.TenantID}='{_context.TenantID}'").Select(spQuery);
